...
*Main reference: [ZeRO paper][zero-paper]*
![](assets/tbl-101_dp_zero2.png)

* You have probably used DistributedDataParallel (DDP) in PyTorch before. Data parallelism is perhaps the most straightforward way to scale training, i.e. by replicating a model across GPUs, and each replica processes a different subset of a minibatch. In Megatron terminology, the batch size of SGD (the minibatch) is referred to as the Global Batch Size (GBS), while the batch size fed to each replica is called the Micro Batch Size (MBS).

* Let's look at R1 and R2 as a basic example of scaling training with DP. **R1** trains a single GPT-2-XL (1.5B parameters) model on 1 GPU with a GBS of 1. **R2** simply replicates R1 by a factor of 4 (DP = 4). Each replica processes 1 MBS (micro-batch), hence GBS = 1 × 4. Since the workload is replicated across GPUs, the memory usage per GPU remains roughly the same as in R1. As for the training time per step, ideally it should stay constant because each replica does the same amount of work in parallel. However, we observe a 14% (~30 ms) increase in step time. This overhead comes from communication, specifically the all-reduce of weight gradients across replicas before the optimizer step. In other words, we pay about 13% more time to process 4× more data. It is not a bad trade off to begin with, it gets better later.

* **When DP is useful?** More often than not, a single GPU can not fit the required batch size even a model can fit within its memory capacity. To illustrate this, **R3** shows that a single 80GB gpu can fit up to a maximum batch of 15, 16 will result in out-of-memory (OOM). With DP=4 in **R5**, the GBS can be scaled proportionally to 60. This is a straightforward way to increase GBS by using more GPUs. Notice that the communication overhead becomes less significant at larger batch sizes. The step time increases from 393 ms to 421 ms, about 6.5%, whereas earlier R2 showed a 14% increase over R1. This is because the communication cost remains roughly constant, i.e. increasing batch size does not change the size of the weight gradients being all-reduced, but it increases the amount of computation. As a result, the ratio of communication to overall step time (comm + comp) decreases with larger batch sizes.

* **Is it possible to achieve GBS of 60 with a single card? Use Gradient Accumulation (GA)**. GA enables larger effective batch sizes on limited memory capacity by accumulating gradients across multiple micro-batches before a weight update. Concretely, without GA, GBS = MBS × DP. With GA, `GBS = (MBS × GA) × DP`. Here, GA refers to number of forward passes, with each replica processing an MBS. How does the training proceed? Recall that a training step consists of: (1) forward, (2) backward, and (3) optimizer step. To preserve equivalent training behavior, each replica runs forward + backward `GA` times to *accumulate* gradients incrementally for a full GBS, and finally performs a single optimizer step. **R4** is a demonstration of GA of 4 with MBS=15, summing up to GBS=60 on a single gpu. That means 4 loops of forward + backward are performed before a weight update. The step time has increased significantly by 3.7× (1470/393) because of the sequential execution of microbatch. It should be apparent that this is a workaround memory constraints and only be used when necessary.
...
* The main redundancy of naive DP is that **each replica maintains a full copy of optimizer states, gradients and parameters (weights)**, which can be a bloated 16-20 bytes per parameter depending on the storage precision. Back in 2019 when ZeRO landed, the top-tier GPU was a V100 with 32 GB. Let's do the math: `32 GB / ~20 bytes ~= 1.6B parameters` is the max model size can be fit and this is even before accounting activations. In other words, with plain DP, we are already out of memory for a small 😂 LLM today (2025) before the training starts. This redundancy not only limits model size, also attenuates batch scaling, which is what DP is meant to do.

* ZeRO asks *"Where Did All the Memory Go?"* We follow the paper analysis on GPT2-XL using Megatron-LM. Let's go to the log of R1, or you can run it again, **R1** is a good setup because we use a minimal input, batch size of 1 so that the monitored memory footprint reflects mostly the usage by model parameters, gradients and model optimizers. Following are the snapshot of the live memory usage and the breakdown by Megatron-LM.
    ```bash
    $ monitor-gpu
...
* Diving into the logs above:
    ...
    4. **Important**: Based on our examination on BF16 training in Megatron-LM, the storage precision as follows:
        ...
        * Each parameter requires 18 bytes/param as opposed to 16 bytes/param in the paper. Therefore, 1.56B*18b=28.08GB of memory needed for param related memory. The observed 33GB usage makes sense because it includes activation memory and temporary buffer needed for input batch size of 1. The theoretical memory footprint (26.7GB) in the log is slightly lower because it only accounts for transformer block but not embedding footprint, whereas the param count includes embedding params. (Yes, we did rootcause it in the code, you can also do the math with log above). 
...
* **What does ZeRO-2 actually entail?** It means the __total__ memory of optimizer states and gradients remains *constant* over any number of DP replicas because they are sharded across replicas instead of being fully replicated. Without ZeRO-2, DP acts as a multiplier on the total memory required for optimizer states and gradients. With ZeRO-2, DP has zero effect on the total memory of optimizer states and gradients, except parameters are replicated (ZeRO-3/TP is the one shards it). This frees up significant memory that can be used to scale batch size or model size. A quick experiment is **R6**, which is **R5** with ZeRO-2 enabled. At DP=4, the memory usage drops from 79GB to 65GB per gpu, close to 18% of savings. 
    
* **Key takeaway: ZeRO-2 enhances DP with sublinear memory growth when scaling DP degree/GBS linearly.**
